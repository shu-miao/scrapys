...
def get_url_m(headers, proxy, key, province, page):
    '''
    :param headers: 请求头
    :param proxy: ip地址
    :param key: 关键词
    ...
    # session = new_session(headers, proxies)
    # strhtml = session.post(url, headers=headers, proxies=proxies)
    strhtml = requests.get(url, headers=headers, proxies=proxies)
    print(f'携带ip：{proxy} 访问 {url}')
...
def get_company(url_m, headers, proxy):
    '''
    :param url_m: 公司详情页地址
    :param headers: 请求头
    :param proxy: ip
    ...
    print(f'携带ip：{proxy} 访问 {url_m}')
    strhtml = requests.get(url_m, headers=headers, proxies=proxies)
    soup = BeautifulSoup(strhtml.text, 'lxml')

    Data = []

    # 公司名
    info_company = soup.select('.index_company-name__LqKlo')
    A, B = '">', '</h1>'
    company_temp = re.findall(f"{A}.+?{B}", str(info_company))
    company_temp = re.sub('">', '', str(company_temp))
    company = re.sub('</h1>', '', str(company_temp))
    Data.append(company)

    # 统一社会信用代码
    info_code = soup.select('.index_copy-text__ri7W6')
    A, B = '">', '</span>'
    code_temp = re.findall(f"{A}.+?{B}", str(info_code))
    code_temp = re.sub('">', '', str(code_temp))
    code = re.sub('</span>', '', str(code_temp))
    Data.append(code)

    # 电话
    info_call = soup.select('.index_detail-tel__fgpsE')
    A, B = '">', '</span>'
    call_temp = re.findall(f"{A}.+?{B}", str(info_call))
    call_temp = re.sub('">', '', str(call_temp))
    call = re.sub('</span>', '', str(call_temp))
    Data.append(call)

    # 法人
    info_legal = soup.select('.detailValue')
    A, B = 'title="', '">'
    legal_temp = re.findall(f"{A}.+?{B}", str(info_legal))
    legal_temp = re.sub('title="', '', str(legal_temp))
    legal = re.sub('">', '', str(legal_temp))
    Data.append(legal)

    # 邮箱
    info_email = soup.select('.index_detail-email__B_1Tq')
    A, B = '">', '</a>'
    email_temp = re.findall(f'{A}.+?{B}', str(info_email))
    email_temp = re.sub('">', '', str(email_temp))
    email = re.sub('</a>', '', str(email_temp))
    Data.append(email)

    # 注册资本
    info_capital = soup.select('.index_detail-text__Ac9Py')
    A, B = '">', '</span>'
    capital_temp = re.findall(f'{A}.+?{B}', str(info_capital))
    capital_temp = re.sub('">', '', str(capital_temp))
    capital = re.sub('</span>', '', str(capital_temp))
    Data.append(capital)

    # 地址
    info_address = soup.select('.index_copy-box__7b6Aq')
    A, B = '">', '</span>'
    address_temp = re.findall(f'{A}.+?{B}', str(info_address))
    address_temp = re.sub('">', '', str(address_temp))
    address = re.sub('</span>', '', str(address_temp))
    Data.append(address)

    # 状态
    info_mode = soup.select('.index_tag-list-content__E8sLp')
    A, B = 'E9FCF1">', '<div'
    mode_temp = re.findall(f'{A}.+?{B}', str(info_mode))
    mode_temp = re.sub('E9FCF1">', '', str(mode_temp))
    mode = re.sub('<div', '', str(mode_temp))
    Data.append(mode)

    companyData = namedtuple('companDate',
                             ['公司名', '统一社会信用代码', '电话', '法人', '邮箱', '注册资本', '地址', '状态'])

    return Data


def new_session(headers, proxies):
    """
    获取session
    :return:
    """
...
def get_co_detail(url, headers, proxies):
    """
    公司详情
    :param url:
    :return:
    ...
    return restext, title, coName


def get_table(url_m, headers, proxy):
    '''
        :param url_m: 公司详情页地址
        :param headers: 请求头
        :param proxy: ip
    ...
    print(f'携带ip: {proxy} 访问: {url_m}')
    # strhtml = requests.get(url_m, headers=headers, proxies=proxies)
    strhtml = session.post(url_m, headers=headers, proxies=proxies)
    soup = BeautifulSoup(strhtml.text, 'lxml')
    info = soup.find_all(".dimHeader_root__XTCLe")
...
# u = get_url_m(headers=get_headers(), ip=get_ip(), key='集成电路设计', province='北京市', page=1)
# print(u)
...
def get_cookie():
    ...
    browser.find_element(By.XPATH, '//*[@id="mobile"]').send_keys('17885024241')
...
def create_csv():
    fo = open('company_tabel2.csv', 'w', newline='')
...
def write_csv(list):
    ...
    :return: none
    '''
    fo = open('company_tabel2.csv', 'a', newline='')
    header = ['关键词', '公司名称', '法人', '登记状态', '成立日期', '统一社会信用代码', '注册资本', '实缴资本',
              '工商注册号',
              '纳税人识别号', '组织机构代码', '行业', '注册地址', '经营范围']
    writer = csv.DictWriter(fo, header)
...
key = ['数据评估']
provice_list1 = ['江西省', '山东省', '河南省', '湖北省', '广东省',
                 '广西壮族自治区', '海南省', '重庆市', '四川省', '贵州省', '云南省', '西藏自治区', '陕西省', '甘肃省',
                 '青海省', '宁夏回族自治区', '新疆维吾尔自治区']
# provice_list1 = ['甘肃省',
...
# url_m = 'https://tianyancha.com'
# ua = UserAgent()
# headers = {
#     'User-Agent': ua.random,
#     'Cookie':'TYCID=2993b5700eaa11efb029a3ac2bd34ac2; CUID=6291b802364d23e0355ddc2a7f1f0091; '
#              'jsid=SEO-BING-ALL-SY-000001; ssuid=1224476340; '
#              'sensorsdata2015jssdkcross=%7B%22distinct_id%22%3A%22304367727%22%2C%22first_id%22%3A%2218f61b125bdc55'
#              '-0a842dcac36385-26001d51-2073600-18f61b125bec70%22%2C%22props%22%3A%7B%22%24latest_traffic_source_type'
...
#              '%22identities%22%3A'
#              '%22eyIkaWRlbnRpdHlfY29va2llX2lkIjoiMThmNjFiMTI1YmRjNTUtMGE4NDJkY2FjMzYzODUtMjYwMDFkNTEtMjA3MzYwMC0xOGY2MWIxMjViZWM3MCIsIiRpZGVudGl0eV9sb2dpbl9pZCI6IjMwNDM2NzcyNyJ9%22%2C%22history_login_id%22%3A%7B%22name%22%3A%22%24identity_login_id%22%2C%22value%22%3A%22304367727%22%7D%2C%22%24device_id%22%3A%2218f61b125bdc55-0a842dcac36385-26001d51-2073600-18f61b125bec70%22%7D; tyc-user-phone=%255B%252218788769290%2522%255D; bannerFlag=true; HWWAFSESTIME=1715652540299; HWWAFSESID=6890bb81ac28d40fb2; csrfToken=Es8hPBScTvCZtyMXj74Fc7qs; searchSessionId=1715652863.25807958; tyc-user-info={%22state%22:%220%22%2C%22vipManager%22:%220%22%2C%22mobile%22:%2218788769290%22%2C%22userId%22:%22304367727%22}; tyc-user-info-save-time=1715657087652; auth_token=eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxODc4ODc2OTI5MCIsImlhdCI6MTcxNTY1NzA4NywiZXhwIjoxNzE4MjQ5MDg3fQ.pWSBGoZfGZDL9yOVv15sCUmTY4nBNDBftZZ794kfqgrr50VQKJqAFhJgYLVZQxEnPESs66ZJnIjf5TYFS8KJVg'}
# proxy = get_proxy()
# proxies = {
#     'http': f'http//{proxy}'
# }
...
# print(get_url_m(headers, proxy, '集成电路设计', '北京市', 1))
# A, B = 'E9FCF1">', '<div'
# print(soup)
# print(info)
# legal_temp = re.findall(f"{A}.+?{B}", str(info))
# print(legal_temp)
# legal = re.sub('E9FCF1">', '', str(legal_temp))
# print(legal)
# legal = re.sub('<div', '', str(legal))
# print(legal)

# url = "https://www.tianyancha.com/search&sessionNo=1715332605.46071440"
# key = '集成电路设计'
# page = 1
# province = base_dict['北京市']
# cacheCode = cacheCode_dict['北京市']
# url_m = f"https://www.tianyancha.com/search?key={key}&base={province}&cacheCode={cacheCode}&pageNum={page}"
# headers = {
#     'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36'}
# strhtml = requests.get(url_m, headers=headers)
# soup = BeautifulSoup(strhtml.text, 'lxml')
# print(soup)
# info = soup.select('.index_name__qEdWi')
# A, B = 'href="', '" tar'
